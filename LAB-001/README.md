...
- Or from the CLI, list trails in your region: `aws cloudtrail describe-trails --region us-east-1` (use your region).
...
- Or from the CLI: `aws logs describe-log-groups --log-group-name-prefix lab-001 --region us-east-1` (use your region).
...
You fix this in the **Console** or with the **CLI**—not Terraform. We'll turn on CloudTrail and send events to CloudWatch Logs so you can query them. Then you re-run the same attacker steps and, as defender, confirm you can see the activity.

**Easiest path: do it in the Console**

A lot of us learn this flow in the console first because you can see what you're doing and AWS will create the CloudWatch log group and the IAM role for you.

1. Go to **CloudTrail** → **Trails** → **Create trail**.
2. **Trail name:** `lab-001-audit-trail` (or whatever you like; we use this name in teardown).
3. **Storage location:** Use the existing S3 bucket—the one Terraform gave you (something like `lab-001-cloudtrail-logs-123456789012`). It's in your Terraform output as `trail_bucket_name`.
4. Under **CloudWatch Logs**, choose to create a new log group. Name it something like `lab-001-cloudtrail-events`. When you do that, the console will offer to create an IAM role so CloudTrail can write to the log group—accept that. It saves you from building the role and policies by hand.
...
**Defender: confirm you can see it**
...
You should see your event. That's the closed loop: we had no logs, we turned on the trail and CloudWatch, we did the same kind of action again, and now we can see it. Fix verified.

**If you prefer the CLI**

You can do the same thing from the command line. You'll need to: create the trail and point it at your S3 bucket, create a log group, create an IAM role that CloudTrail can assume to write to that log group (with the right trust policy and permissions to write logs), and put a resource policy on the log group so CloudTrail is allowed to write. Then update the trail with the log group ARN and role ARN and start logging. AWS documents this in [Sending CloudTrail events to CloudWatch Logs](https://docs.aws.amazon.com/awscloudtrail/latest/userguide/send-cloudtrail-events-to-cloudwatch-logs.html)—the "Create the role" and "Add permissions" sections walk through the role and the log group resource policy. Once the trail is created and logging, re-run the create-user command and check Event history or Logs Insights as above.
...
Terraform didn't create the trail or the log group—you did that in the console (or CLI). So you have to remove them yourself before destroying the Terraform bucket.
